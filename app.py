...
class Table(db.Model):
    ...
    qr_secret = db.Column(db.String(50), unique=True, nullable=False, default=lambda: os.urandom(16).hex())
...
def add_tables():
    if request.method == 'POST':
        ...
        # Dodaj stoliki do bazy danych
        for i in range(1, table_count + 1):
            if not Table.query.get(i):  # Sprawdź, czy stolik już istnieje
                new_table = Table(id=i, qr_code=f"table_{i}")
                db.session.add(new_table)

                # Generowanie bezpiecznego linku do menu z użyciem `qr_secret`
                link = url_for('menu', table_id=i, qr_secret=new_table.qr_secret, _external=True)
                
                # Generowanie QR kodu
                img = generate_qr_code(link)

                # Zapis QR kodu do pliku
                filename = f"static/qr_codes/table_{i}.png"
                img.save(filename)

        db.session.commit()
        flash(f'Dodano {table_count} stoliki.', 'success')
        return redirect(url_for('add_tables'))

    tables = Table.query.all()  # Pobierz wszystkie stoliki
